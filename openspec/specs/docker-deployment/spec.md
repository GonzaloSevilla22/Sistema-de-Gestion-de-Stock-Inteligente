...
Provides Docker-based packaging and orchestration for the full stack. Includes a production-ready `backend/Dockerfile` (python:3.11-slim + uvicorn), a multi-stage `frontend/Dockerfile` (node:20-alpine build + nginx:alpine serve), and a `docker-compose.yml` that starts both services with a single command.
