# Spec: movement-schema

## Purpose

Defines the Pydantic v2 schemas for StockMovement API input/output: `MovementCreate` (POST body) and `MovementResponse` (API response). The `date` field is server-assigned and excluded from the create schema (RN-MV-06).
