...
Defines the Pydantic v2 schemas for Product API input/output: `ProductCreate` (POST body), `ProductUpdate` (PATCH body), and `ProductResponse` (API response with computed `status` field).
