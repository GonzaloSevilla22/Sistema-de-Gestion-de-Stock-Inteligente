# Capability: ai-recommendation

## Purpose

Provides a stateless `POST /ai/recommendation` endpoint that calls an external LLM (Claude, Gemini, or OpenAI — configurable via `AI_PROVIDER`) and returns a natural-language stock management recommendation in Spanish. Falls back gracefully if the LLM API is unavailable.
...
#### Scenario: Test verifies OpenAI path with mock client
- **WHEN** `get_recommendation` is called with a mock OpenAI client that returns a fixed response
- **THEN** the function returns the mocked message
